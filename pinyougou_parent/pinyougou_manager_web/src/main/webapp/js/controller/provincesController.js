	...
	var app = new Vue({
		...
		methods:{
			...
			findPage:function (pageNo) {
				axios.post("../provinces/findPage.do?pageNo="+pageNo+"&pageSize="+10,this.searchEntity)
			...
			//新增
			add:function () {
				var url = "../provinces/add.do";
				if(this.entity.id != null){
					url = "../provinces/update.do";
				}
			...
			//跟据id查询
			getById:function (id) {
				axios.get("../provinces/getById.do?id="+id).then(function (response) {
					app.entity = response.data;
				})
			},
			//批量删除数据
			dele:function () {
				axios.get("../provinces/delete.do?ids="+this.ids).then(function (response) {
					if(response.data.success){
						//刷新数据
						app.findPage(app.pageNo);
						//清空勾选的ids
