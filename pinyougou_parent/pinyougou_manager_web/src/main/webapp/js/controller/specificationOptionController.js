...
window.onload=function () {
	...
	var app = new Vue({
		...
		methods:{
			//查询所有
			findAll:function () {
				axios.get("../specificationOption/findAll.do").then(function (response) {
					//vue把数据列表包装在data属性中
			...
			findPage:function (pageNo) {
				axios.post("../specificationOption/findPage.do?pageNo="+pageNo+"&pageSize="+10,this.searchEntity)
			...
			//新增
			add:function () {
				var url = "../specificationOption/add.do";
				if(this.entity.id != null){
					url = "../specificationOption/update.do";
				}
			...
			//跟据id查询
			getById:function (id) {
				axios.get("../specificationOption/getById.do?id="+id).then(function (response) {
