...
window.onload=function () {
	...
	var app = new Vue({
		...
		methods:{
			//查询所有
			findAll:function () {
				axios.get("../address/findAll.do").then(function (response) {
					//vue把数据列表包装在data属性中
			...
			findPage:function (pageNo) {
				axios.post("../address/findPage.do?pageNo="+pageNo+"&pageSize="+10,this.searchEntity)
			...
			//批量删除数据
			dele:function () {
				axios.get("../address/delete.do?ids="+this.ids).then(function (response) {
					if(response.data.success){
						//刷新数据
						app.findPage(app.pageNo);
						//清空勾选的ids
